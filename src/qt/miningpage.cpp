...
void MiningPage::minerError(QProcess::ProcessError error)
{
    ...
    {
        reportToList("Miner failed to start. Make sure you have the minerd executable and libraries in the same directory as TittieCoin-Qt.", ERROR, NULL);
    }
}
...
{
    if (getMiningType() == ClientModel::SoloMining)
        reportToList("Solo motorboating stopped.", ERROR, NULL);
    else
...
void MiningPage::minerStarted()
{
    if (!minerActive)
        if (getMiningType() == ClientModel::SoloMining)
            reportToList("Solo motorboating started.", ERROR, NULL);
        else
...
{
    ui->startButton->setText(minerActive ? "Stop Motorboating" : "Start Motorboating");
